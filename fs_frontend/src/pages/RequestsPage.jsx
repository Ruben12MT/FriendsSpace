...
export default function RequestsPages() {
  ...
  const filterBtnSx = (type) => ({
    ...
    color:
      filter === type ? (isDark ? "#1a1200" : "#ffffff") : theme.primaryText,
  ...
  useEffect(() => {
    ...
    const handleNewRequest = (payload) => {
      ...
      if (data.is_report) {
        ...
        const fmt = {
          ...data,
          sender: data.sender || { id: data.sender_id, name: "Usuario" },
          created_at: data.created_at || new Date().toISOString(),
        };
        setAllUserRequests((prev) =>
          prev.find((r) => r.id === fmt.id)
            ? prev
            : sortRequests([fmt, ...prev]),
        );
      }
    };
  ...
  const onAction = async (idReq, action) => {
    try {
      ...
      if (res.data.ok) {
        ...
        const update = (prev) =>
          sortRequests(
            prev.map((r) => (r.id === idReq ? { ...r, ...changes } : r)),
          );
      ...
      >
        {requestsToShow.length > 0 ? (
          ...
          <Box sx={{ textAlign: "center", mt: 8, opacity: 0.5 }}>
            <NotificationsIcon
              sx={{ fontSize: 48, color: theme.mutedText, mb: 1 }}
            />
