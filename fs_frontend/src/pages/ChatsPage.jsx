...
export default function ChatsPage() {
  ...
  const mapConversations = (datos, currentUserId) => {
    return datos.map((connection) => {
      const myUserConnection = connection.user_connections?.find((uc) => uc.user?.id === currentUserId);
      const friendUserConnection = connection.user_connections?.find((uc) => uc.user?.id !== currentUserId);
      const isBlocked = connection.status === "BLOCKED";
      const blockedByMe = isBlocked && myUserConnection?.blocked_by === currentUserId;
      const friendRole = friendUserConnection?.user?.role;
      // Es un chat de reporte si el otro usuario es USER (y yo soy admin)
  ...
  // Conversaciones filtradas según la vista activa
  const conversationList = isAdmin
    ? allConversations.filter((c) => activeView === "reportes" ? c.isReport : !c.isReport)
    : allConversations;
    ...
    async function fetchConversationList() {
      ...
      try {
        ...
        if (res.data.ok) {
          ...
          if (navigationState?.openConnectionId) {
            const target = mapped.find((c) => c.connectionId === navigationState.openConnectionId);
            if (target) {
              // Si el target es un reporte y somos admin, activamos la vista de reportes
  ...
  // Al cambiar de vista cerramos la conversación activa si no pertenece a esa vista
  const handleViewChange = (view) => {
    setActiveView(view);
    if (activeConversation) {
      const pertenece = view === "reportes" ? activeConversation.isReport : !activeConversation.isReport;
      if (!pertenece) {
  ...
  const handleUnblockAndContinue = async () => {
    try {
      ...
      if (res.data.ok) {
        const mapped = mapConversations(res.data.datos, loggedUser.id);
        setAllConversations(mapped);
        const updated = mapped.find((c) => c.connectionId === activeConversation.connectionId);
        if (updated) setActiveConversation(updated);
      }
    } catch (error) { console.error(error); }
    finally { setBlockDialog({ open: false, type: null }); }
  };
  ...
  return (
    ...
    >
      {/* Panel lateral */}
      <Box sx={{
        width: CONVERSATION_PANEL_WIDTH, flexShrink: 0,
        borderRight: `1px solid ${borderColor}`, background: sidebarBackground,
        display: "flex", flexDirection: "column", overflow: "hidden",
      }}>

        {/* Switch Chats / Reportes para admins */}
        {isAdmin ? (
          <Box sx={{ display: "flex", borderBottom: `1px solid ${borderColor}`, flexShrink: 0 }}>
            <Box
              onClick={() => handleViewChange("chats")}
              sx={{
                flex: 1, py: 1.25, display: "flex", flexDirection: "column",
                alignItems: "center", gap: 0.25, cursor: "pointer",
                borderBottom: activeView === "chats" ? `2px solid ${accentColor}` : "2px solid transparent",
            ...
            >
              ...
              <Typography sx={{ fontSize: "0.7rem", fontWeight: 600, color: primaryTextColor }}>
            ...
            <Box
              ...
              sx={{
                flex: 1, py: 1.25, display: "flex", flexDirection: "column",
                alignItems: "center", gap: 0.25, cursor: "pointer",
                borderBottom: activeView === "reportes" ? "2px solid #f44336" : "2px solid transparent",
                opacity: activeView === "reportes" ? 1 : 0.5,
                transition: "all 0.15s",
                "&:hover": { opacity: 1 },
              }}
            >
              <ReportIcon sx={{ fontSize: 18, color: activeView === "reportes" ? "#f44336" : primaryTextColor }} />
              <Typography sx={{ fontSize: "0.7rem", fontWeight: 600, color: activeView === "reportes" ? "#f44336" : primaryTextColor }}>
          ...
          <Box sx={{ px: 2, py: 1.5, borderBottom: `1px solid ${borderColor}`, flexShrink: 0 }}>
            <Typography sx={{ fontWeight: 700, color: primaryTextColor, fontSize: "1rem" }}>Chats</Typography>
          </Box>
        )}
          ...
          {conversationList.length === 0 ? (
            <Box sx={{ p: 3, textAlign: "center" }}>
              <Typography sx={{ color: subtleTextColor, fontSize: "0.85rem" }}>
                {activeView === "reportes" ? "No tienes chats de reporte" : "Aún no tienes chats"}
              </Typography>
            </Box>
          ) : (
      ...
      {!activeConversation ? (
        <Box sx={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: chatAreaBackground }}>
          <Box sx={{ textAlign: "center", opacity: 0.35 }}>
            ...
            <Typography sx={{ color: subtleTextColor, fontSize: "0.85rem" }}>Elige una conversación de la lista</Typography>
          </Box>
        </Box>
      ) : (
        <Box sx={{ flex: 1, display: "flex", flexDirection: "column", background: chatAreaBackground, overflow: "hidden" }}>
          ...
          {/* Header del chat */}
          <Box sx={{
            px: 2, py: 1.25, borderBottom: `1px solid ${borderColor}`,
            display: "flex", alignItems: "center", gap: 1.5,
            background: sidebarBackground, flexShrink: 0,
          }}>
            <Avatar
              src={activeConversation.friend?.url_image || "/no_user_avatar_image.png"}
              sx={{ width: 34, height: 34, filter: isActiveConversationBlocked ? "grayscale(100%)" : "none", opacity: isActiveConversationBlocked ? 0.6 : 1 }}
            />
            <Box>
              <Box display="flex" alignItems="center" gap={0.75}>
                <Typography sx={{ fontWeight: 700, color: primaryTextColor, lineHeight: 1.2 }}>
              ...
              {isActiveConversationBlocked && (
                <Typography sx={{ fontSize: "0.7rem", color: "#f44336" }}>
                  {activeConversation.blockedByMe ? "Usuario bloqueado" : "No disponible"}
                </Typography>
              )}
              {activeConversation.isReport && !isActiveConversationBlocked && (
                <Typography sx={{ fontSize: "0.7rem", color: "#f44336" }}>Chat de investigación</Typography>
              )}
            </Box>
          </Box>
          ...
          {(replyToMessage || editingMessage) && (
            <Box sx={{ px: 2, py: 0.75, background: sidebarBackground, borderTop: `1px solid ${borderColor}`, display: "flex", alignItems: "center", gap: 1, flexShrink: 0 }}>
              <Box sx={{ flex: 1, px: 1.5, py: 0.75, borderLeft: `3px solid ${accentColor}`, borderRadius: "6px", background: `${accentColor}12` }}>
              ...
              <IconButton size="small" onClick={() => { setReplyToMessage(null); setEditingMessage(null); setMessageText(""); }} sx={{ color: subtleTextColor }}>
          ...
          {/* Input */}
          <Box sx={{ px: 2, py: 1.25, borderTop: `1px solid ${borderColor}`, background: sidebarBackground, display: "flex", alignItems: "flex-end", gap: 1, flexShrink: 0 }}>
            {!editingMessage && (
              <>
                <input
                ...
                <Tooltip title="Adjuntar archivo">
                  <IconButton size="small" onClick={() => fileInputRef.current?.click()} sx={{ color: subtleTextColor, "&:hover": { color: accentColor } }}>
            ...
            <TextField
              inputRef={textInputRef} fullWidth multiline maxRows={4}
              placeholder={isActiveConversationBlocked ? "Conversación no disponible" : "Escribe un mensaje..."}
              value={messageText}
              onChange={(e) => setMessageText(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendTextMessage(); } }}
              onClick={() => { if (isActiveConversationBlocked) checkBlockedAction(); }}
              sx={{
                "& .MuiOutlinedInput-root": {
                  borderRadius: "12px", background: chatAreaBackground,
                  "& fieldset": { borderColor: isActiveConversationBlocked ? "#f4433640" : `${accentColor}40` },
                  "&:hover fieldset": { borderColor: isActiveConversationBlocked ? "#f44336" : accentColor },
                  "&.Mui-focused fieldset": { borderColor: isActiveConversationBlocked ? "#f44336" : accentColor },
                },
                "& .MuiInputBase-input": { color: isActiveConversationBlocked ? subtleTextColor : primaryTextColor, fontSize: "0.875rem", cursor: isActiveConversationBlocked ? "not-allowed" : "text" },
              }}
            />
            ...
            <Tooltip title={editingMessage ? "Guardar cambios" : "Enviar"}>
              <span>
                <IconButton
                  onClick={sendTextMessage} disabled={!messageText.trim() || isSending}
                  sx={{
                    background: messageText.trim() ? `linear-gradient(135deg, ${accentColor}, ${theme.variantBack})` : "transparent",
                    color: messageText.trim() ? "#fff" : subtleTextColor,
                    borderRadius: "12px", width: 40, height: 40,
                    "&:hover": { opacity: 0.85 }, "&.Mui-disabled": { background: "transparent" },
                  }}
                >
      ...
      {contextMenu.visible && contextMenu.message && (
        <ChatContextMenu
          ...
          onReply={startReplyingToMessage} onEdit={startEditingMessage}
          onDelete={deleteMessage} onClose={closeContextMenu}
        />
      )}
      ...
      <Dialog
        ...
        PaperProps={{ sx: { borderRadius: "16px", background: sidebarBackground, border: `1px solid ${borderColor}`, minWidth: 320 } }}
      >
        <DialogTitle sx={{ color: primaryTextColor, fontWeight: 700, pb: 1 }}>
          <Box display="flex" alignItems="center" gap={1}>
            <BlockIcon sx={{ color: "#f44336", fontSize: 20 }} />
            {blockDialog.type === "BLOCKED_BY_ME" ? "Usuario bloqueado" : "Acción no disponible"}
          </Box>
        </DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ color: subtleTextColor, fontSize: "0.875rem" }}>
        ...
        <DialogActions sx={{ px: 2, pb: 2, gap: 1 }}>
          <Button onClick={() => setBlockDialog({ open: false, type: null })} sx={{ color: subtleTextColor, textTransform: "none", borderRadius: "8px" }}>
            Cancelar
          </Button>
          {blockDialog.type === "BLOCKED_BY_ME" && (
            <Button onClick={handleUnblockAndContinue} variant="contained"
              sx={{ background: `linear-gradient(135deg, ${accentColor}, ${theme.variantBack})`, color: "#fff", textTransform: "none", borderRadius: "8px", fontWeight: 600 }}>
              Desbloquear
            </Button>
          )}
          {blockDialog.type === "BLOCKED_BY_THEM" && (
            <Button onClick={() => setBlockDialog({ open: false, type: null })} variant="contained"
              sx={{ background: `linear-gradient(135deg, ${accentColor}, ${theme.variantBack})`, color: "#fff", textTransform: "none", borderRadius: "8px", fontWeight: 600 }}>
              Entendido
            </Button>
          )}
