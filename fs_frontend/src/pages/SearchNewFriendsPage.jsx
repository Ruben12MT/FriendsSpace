...
export default function SearchNewFriendsPage() {
  ...
  const toggleButtonSx = {
    border: "none", borderRadius: "8px !important", color: textMuted, background: "transparent",
    "&.Mui-selected": { color: textMain, background: `${accent}20`, "&:hover": { background: `${accent}30` } },
    "&:hover": { background: `${accent}10` }, px: 1.25,
  };
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(() => { fetchUsers(1, true); }, 400);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting && hasMore && !isLoadingMore && !isLoading) fetchUsers(page + 1);
    }, { threshold: 0.1 });
  ...
  const handleReset = () => { setQuery(""); setSelectedInterests([-1]); };
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2.5 }}>
          <Box>
            <Typography sx={{ fontWeight: 700, fontSize: { xs: "1.2rem", md: "1.5rem" }, color: textMain, lineHeight: 1.2 }}>
              {loggedUser?.role === "DEVELOPER" || loggedUser?.role === "ADMIN" ? "Buscar usuarios" : "Encuentra tu friend"}
            </Typography>
