    ...
    return (
      <Tooltip title={label} placement="right">
        <Box sx={{ position: "relative", width: "100%", display: "flex", justifyContent: "center" }}>
          {active && (
            <Box sx={{ position: "absolute", left: 0, top: "50%", transform: "translateY(-50%)", width: 3, height: 22, borderRadius: "0 3px 3px 0", background: accent }} />
          )}
          <IconButton
            onClick={() => navigate(path)}
            sx={{ width: 44, height: 44, borderRadius: "12px", color: active ? accent : fg, background: active ? activeBg : "transparent", border: `1px solid ${active ? accent + "40" : "transparent"}`, transition: "all 0.15s ease", "&:hover": { background: hoverBg, color: accent } }}
          >
            {icon}
          </IconButton>
        </Box>
    ...
    <Box sx={{ display: "flex", minHeight: "100vh" }}>
      <Box sx={{ position: "fixed", top: 0, left: 0, right: 0, height: TOPBAR_H, zIndex: 1200, background: bg, borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", pl: `${SIDEBAR_W}px`, pr: 2, gap: 1 }}>
        ...
        <Tooltip title="Notificaciones">
          <IconButton onClick={() => navigate("/app/requests")} sx={{ width: 36, height: 36, borderRadius: "10px", color: estaEnRequests ? accent : fg, background: estaEnRequests ? activeBg : "transparent", border: `1px solid ${estaEnRequests ? accent + "50" : "transparent"}`, transition: "all 0.15s", "&:hover": { background: hoverBg, color: accent } }}>
            <Badge badgeContent={unreadCount} color="error" sx={{ "& .MuiBadge-badge": { fontSize: "0.6rem", minWidth: 15, height: 15, padding: "0 3px" } }}>
      ...
      <Menu anchorEl={anchorElUser} open={Boolean(anchorElUser)} onClose={() => setAnchorElUser(null)} anchorOrigin={{ vertical: "bottom", horizontal: "right" }} transformOrigin={{ vertical: "bottom", horizontal: "left" }}
        ...
        <MenuItem onClick={() => { setAnchorElUser(null); navigate("/app/" + loggedUser?.id); }} sx={{ gap: 1.5, py: 1.2, color: fg, fontSize: "0.85rem", "&:hover": { background: hoverBg, color: accent } }}>
          <AccountCircleIcon fontSize="small" sx={{ opacity: 0.65 }} />
          Ver perfil
        </MenuItem>
        <Divider sx={{ borderColor: border }} />
        <MenuItem onClick={logout} sx={{ gap: 1.5, py: 1.2, color: "#f44336", fontSize: "0.85rem", "&:hover": { background: "rgba(244,67,54,0.08)" } }}>
